		...
		while($row = $query->fetch_assoc()) {
			...
			if (decryptthis($row["email"],$key)==$_POST['email']){
				...
				if ($check){
					...
					$_SESSION['id'] = $row['id'];
					$encrypted = encryptthis($row['id'],$key);
					$_SESSION['username'] = $row['name'];
					$uip = $_SERVER['REMOTE_ADDR'];
					$status = 1;
					$log = mysqli_query($con, "insert into userlog(userEmail,userip,status) values('" . $encrypted . "','$encrypted','$status')");
...
<head>
	...
	<title>DC Store|Sign-in/Signup</title>
